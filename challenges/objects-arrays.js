...
function createDino(name, diet, weight, length, period){
  const dino = {};
    dino.name = name;
    dino.diet = diet;
    dino.weight = weight;
...
const trex = createDino("tyrannosaurus", "carnivorous", "7000kg", "12m", "Late Cretaceous");

// stegosaurus, herbivorous, 2000kg, 9m, Late Jurassic

const stego = createDino("stegosaurus", "herbivorous", "2000kg", "9m", "Late Jurassic");

// velociraptor, carnivorous, 15kg, 1.8m, Late Cretaceous

const raptor = createDino("velociraptor", "carnivorous", "15", "1.8m", "Late Cretaceous");
...
graduates.forEach(function(item){
  var str = `${item.first_name} ${item.email}`;
  contactInfo.push(str);
})
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = universities.filter((item) => {return item.includes("Uni") });
...
zooAnimals.forEach((item) => {
  let ans = `Name: ${item.animal_name}, Scientific: ${item.scientific_name}.`;
  displayNames.push(ans);
})
...
const populationTotal = zooAnimals.reduce((total, item) => {return total + item.population}, 0);
